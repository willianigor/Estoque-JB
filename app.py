...
def backup_database():
    """Faz backup do banco de dados antes de operações críticas"""
...
def init_db() -> None:
    ...
    cur.executescript(
        ...
        CREATE TABLE IF NOT EXISTS variants (
            ...
            custo_unitario REAL  -- custo específico da variante (opcional)
        );
        ...
        -- Nesta view de valor, usamos COALESCE do custo da variante > custo do produto
...
def migrate_db() -> None:
    ...
    # custo_unitario em variants (NOVO)
    try:
    ...
    # recria a view stock_value_view com COALESCE(v.custo_unitario, p.custo_unitario)
    cur.execute("DROP VIEW IF EXISTS stock_value_view")
    cur.execute("""
        ...
        GROUP BY v.id, v.sku;
    """)
    st.info("✓ View stock_value_view (COALESCE custo variante/produto) atualizada")
...
def create_variant(category: str, subtype: str, color: str, size: str,
                   sku_base: Optional[str] = None,
                   sku_override: Optional[str] = None,
                   custo_unitario_produto: float = 0,
                   custo_unitario_variante: Optional[float] = None) -> Tuple[bool, str]:
    ...
    try:
        cur.execute(
            "INSERT INTO variants(product_id, color, size, sku, custo_unitario) VALUES(?,?,?,?,?)",
...
def update_variant(old_sku: str, new_sku: str, category: str, subtype: str, color: str, size: str,
                   sku_base: Optional[str] = None,
                   custo_unitario_produto: Optional[float] = None,
                   custo_unitario_variante: Optional[float] = None) -> Tuple[bool, str]:
    """Atualiza dados da variante e permite alterar custo da VARIANTE e/ou do PRODUTO."""
    try:
        backup_database()
        con = get_conn()
        cur = con.cursor()
        ...
        cur.execute(
            "UPDATE variants SET sku=?, color=?, size=?, product_id=?, custo_unitario=? WHERE id=?",
            (new_sku, color.strip(), size.strip(), new_product_id,
             float(custo_unitario_variante) if custo_unitario_variante is not None else None,
             variant_id)
        )
...
def update_custo_unitario(category: str, subtype: str, novo_custo: float) -> Tuple[bool, str]:
    """Atualiza custo no PRODUTO (categoria/subtipo). As variantes podem ter custo próprio que se sobrepõe."""
    try:
        backup_database()
        con = get_conn()
        cur = con.cursor()
...
def get_variant_details(sku: str) -> Optional[dict]:
    ...
    try:
        cur.execute("""
            SELECT v.sku, p.category, p.subtype, v.color, v.size, v.id, p.id, p.sku_base,
    ...
    if row:
        return {
            ...
            'custo_unitario_variante': row[9]  # pode ser None
        }
    return None
...
def list_variants_df() -> pd.DataFrame:
    ...
    try:
        ...
        SELECT v.id, v.sku, p.category, p.subtype, v.color, v.size,
               ...
               v.custo_unitario AS custo_unitario_variante,
               p.id as product_id
        FROM variants v
        JOIN products p ON p.id=v.product_id
...
def get_sales_data(days: Optional[int] = None) -> pd.DataFrame:
    con = get_conn()
    # Valor ao custo calculado com custo da VARIANTE (se houver) ou do PRODUTO
    sql = """
    ...
    if days:
        ...
        sql += " AND m.ts >= ?"
        params.append(ts_min)
    sql += " GROUP BY p.category, p.subtype, v.color, v.size"
    sql += " ORDER BY quantidade_vendida DESC"
    return pd.read_sql_query(sql, con, params=params)

# ==========================================
# SKU Mapping helpers (usados pelo parser do PDF)
# ==========================================
def get_sku_mapping(sku_pdf_norm: str) -> Optional[str]:
    """
    Retorna o SKU do estoque (variants.sku) mapeado p/ um sku_pdf normalizado.
    Usa chave sanitizada para buscar na tabela, mas SEM sanitizar o valor (preserva case original).
    """
    key_pdf = sanitize_sku(sku_pdf_norm)
    con = get_conn()
    cur = con.cursor()
    # 1) Tabela de mapeamento (valor em CASE ORIGINAL)
    try:
        ...
        pass
    # 2) Casamento direto por normalização (retorna SKU ORIGINAL da variants)
    vdf = list_variants_df()
...
# ==========================================
# PDF Parser (corrigido e robusto p/ UpSeller)
# ==========================================
def processar_pdf_vendas(pdf_file) -> Tuple[bool, List[dict], str]:
    """
    Parser robusto p/ PDF (UpSeller) — v3
    """
    try:
        ...
        def maybe_int(txt: str, next_char: Optional[str] = None) -> Optional[int]:
            ...
            if not re.fullmatch(r"\d{1,3}", txt): return None
            ...
            for m in sku_pattern.finditer(compact):
                ...
                if qty_val is not None:
                    ...
                    if key not in vistos:
                        ...
                        movimentos.append({
                            "sku_pdf": sku_n,
                            "sku": mapped or sku_n,  # ORIGINAL se houver
...
# ==========================================
# Utilitário opcional (stub)
# ==========================================
def criar_mapeamentos_automaticos() -> int:
    """Stub — espaço para regras automáticas, se desejar implementar depois."""
    return 0
...
with st.sidebar:
    ...
    if st.button("🔄 Forçar Migração do Banco"):
        migrate_db()
        st.sidebar.success("Migração executada com sucesso!")
        st.rerun()
    ...
    if st.button("🧠 Criar Mapeamentos Automáticos"):
        total = criar_mapeamentos_automaticos()
        st.success(f"✅ {total} mapeamentos criados automaticamente!")
...
# -------- Cadastrar Variante --------
    ...
    custo_unitario_produto = st.number_input("Custo Unitário (PRODUTO) R$", min_value=0.0, value=0.0, step=0.01,
                                             help="Define/atualiza o custo padrão do produto (categoria/subtipo)")
    custo_unitario_variante = st.number_input("Custo Unitário (VARIANTE) R$ (opcional)", min_value=0.0, value=0.0, step=0.01,
                                              help="Se informado > 0, esta variante usará este custo (não afeta as outras)")
    ...
    if st.button("Criar variante", type="primary"):
        ...
        ok, msg = create_variant(category, subtype, color, size, sku_base,
                                 sku_override, custo_unitario_produto, cvar)
        if ok:
    ...
    if up is not None:
        ...
        if not ok or not movimentos:
            ...
            def status_row(after: int) -> str:
                if after < 0: return "FICA NEGATIVO"
                if after == 0: return "ZERA ESTOQUE"
                return "OK"
            ...
            if st.button("Aplicar baixas (venda_pdf)", type="primary"):
                ...
                for _, r in edited.iterrows():
                    ...
                    # usar corrigida; se faltar, cai na lida
...
# -------- Estoque Atual --------
    ...
    if not df.empty and 'valor_estoque' in df.columns:
        ...
        with col1: st.metric("Total de itens", total_itens)
        with col2: st.metric("Total de unidades", total_unidades)
        with col3: st.metric("Valor total do estoque", f"R$ {valor_total_estoque:,.2f}")
        with col4:
    ...
    if df.empty:
        ...
        with col1: st.metric("Itens críticos", total_criticos)
        with col2: st.metric("Estoques negativos", total_negativos)
...
# -------- Editar Variante --------
    ...
    if current_sku:
        ...
        with col1: category = st.text_input("Categoria", det["category"])
        with col2: subtype = st.text_input("Subtipo", det["subtype"])
        with col3: color = st.text_input("Cor", det["color"])
        with col4: size = st.text_input("Tamanho", det["size"])
        with col5: sku_base = st.text_input("SKU Base", det["sku_base"] or "")

        # Custos
        c1, c2 = st.columns(2)
        with c1:
            custo_unitario_produto = st.number_input("Custo Unitário (PRODUTO) R$", min_value=0.0,
                                                     value=float(det.get("custo_unitario_produto", 0) or 0), step=0.01,
                                                     help="Custo padrão do tipo/subtipo. Variantes podem ter custo próprio.")
        with c2:
            cur_val = det.get("custo_unitario_variante", None)
            custo_unitario_variante = st.number_input("Custo Unitário (VARIANTE) R$ (opcional)", min_value=0.0,
                                                      value=float(cur_val if cur_val is not None else 0.0), step=0.01,
                                                      help="Se > 0, substitui o custo do produto apenas para esta variante.")

        new_sku = st.text_input("Novo SKU", det["sku"])
        if st.button("Salvar alterações", type="primary"):
            cvar = custo_unitario_variante if custo_unitario_variante > 0 else None
            ok, msg = update_variant(det["sku"], new_sku, category, subtype, color, size,
                                     sku_base if sku_base else None,
                                     custo_unitario_produto, cvar)
...
# -------- Gerenciar SKU Base --------
    ...
    col1, col2, col3 = st.columns(3)
    with col1: category = st.text_input("Categoria")
    with col2: subtype = st.text_input("Subtipo")
    with col3: new_base = st.text_input("Novo SKU Base (ex.: MOL-CARECA)")
...
# -------- Valor do Estoque --------
    ...
    if df_estoque.empty:
        ...
        col1, col2, col3, col4 = st.columns(4)
        with col1: st.metric("Valor Total do Estoque", f"R$ {valor_total:,.2f}")
        with col2: st.metric("Total de Itens", total_itens)
        with col3: st.metric("Total de Unidades", total_unidades)
        with col4:
...
# -------- Gráfico de Vendas --------
    ...
    if df_vendas.empty:
        ...
        with m1: st.metric("Unidades vendidas", total_qtd)
        with m2: st.metric("Valor ao custo", f"R$ {total_val:,.2f}")
        with m3: st.metric("Registros (linhas) de venda", total_regs)
        ...
        if not df_top.empty:
            fig1 = px.bar(
                df_top, x="quantidade_vendida", y="produto", orientation="h",
            ...
            if modo_valor == "Valor ao Custo":
                fig2 = px.bar(
                    ...
                    x="valor_total_vendido", y="produto", orientation="h",
        ...
        if not df_prod.empty:
            ...
            fig_tam = px.bar(
                df_tam, x="size", y="quantidade_vendida",
        ...
        with cti1:
            fig_items_q = px.bar(
                ...
                x="quantidade_vendida", y="item", orientation="h",
        ...
        with cti2:
            fig_items_v = px.bar(
                ...
                x="valor_total_vendido", y="item", orientation="h",
